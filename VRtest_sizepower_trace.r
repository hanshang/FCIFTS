## Description: Simulation codes for the size-power properties of the variance-ratio trace test.
...
x_mat[,i]=mu + y1[i]*LBF[,1]+y2[i]*LBF[,2]+y3[i]*LBF[,3]+y4[i]*LBF[,4]+y5[i]*LBF[,5]+ eta
    ...
    test=(TTT^(2*dalpha))*cumsum(EV) ;    CV=CVV[4,index]
...
if (TTT==nobs[4]){DD1d = append(DD1d,testresult)}# ; if (TTT==nobs[5]){DD1e = append(DD1e,sss)}
	...
	lrx1 = crossprod(t(fdtmp1[1:kdim,]))
	EV=sort(eigen(lrx0%*%solve(lrx1),only.values = TRUE)$values)
    test=(TTT^(2*dalpha))*cumsum(EV) ;    CV=CVV[5,index]
...
if (TTT==nobs[4]){DD2d = append(DD2d,testresult)}# ; if (TTT==nobs[5]){DD1e = append(DD1e,sss)}
	}
